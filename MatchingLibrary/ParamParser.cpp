...
void *ParamParser::readFile(void *document) {
	...
	if (!ifs.is_open()) {
		exc::SequenceMatchingException::genererException(
			"Parameters XML Reading error : Unable to open the file", ERREUR_FILE, __LINE__);
	}
	...
	std::vector<tools::Parameters*> * parametersList = new std::vector<tools::Parameters*>();

	rapidxml::xml_node<> *list_node;
	if ((list_node = docr.first_node("Lists")) == 0) {
		exc::SequenceMatchingException::genererException(
			"Parameters XML Reading error : Lists node was not found", ERREUR_FILE, __LINE__);
	}
	// Take the root
	...
	if (comp_node == 0) {
		exc::SequenceMatchingException::genererException(
			"Parameters XML Reading error : Comparaison node was not found", ERREUR_FILE, __LINE__);
	}
	do {
		// Création objets parameters
		...
		if (mvm_node != 0) {
			...
			parameters->putValue(parameters->MVM_ELASTICITY, (float) elastic);
		}
		...
		if (fsm_node != 0) {
			...
			if (elasticity_fsm_attr != 0) {
				...
				parameters->putValue(parameters->FSM_ELASTICITY, (float) elasticity);
			}
			...
			if (typeOfResult_fsm_attr != 0) {
				...
				parameters->putValue(parameters->FSM_TYPE_RESULT, (float) typeofResult);
			}
			...
			if (nbFirstColRes_fsm_attr != 0) {
				...
				parameters->putValue(parameters->FSM_FIRST_COL_FOR_RESULT, (float) firstColRes);
			}
			...
			if (nbMinPerLine_fsm_attr != 0) {
				...
				parameters->putValue(parameters->FSM_NUMBER_ELEMENTS_BY_LINE, (float) minperline);
			}
			...
			if (standartDeviationNumber_fsm_attr != 0) {
				...
				parameters->putValue(parameters->FSM_STANDARD_DEVIATION_NUMBER, (float) standardDev);
			}
		}

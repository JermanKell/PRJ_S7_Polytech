...
std::string tools::Correspondence::format(model::Sequence *sTarget, model::Sequence *sRef, ResultCorrespondence * result)
{
	stringstream ret;
	ret << result->distance << ": ";
	unsigned int it1 = 0;
...
std::string tools::Correspondence::format(model::Sequence *sTarget, model::Sequence *sRef, vector<ResultCorrespondence>* result)
{
	stringstream ret;
...
void Correspondence::parametrerVectors(model::Sequence *s1, model::Sequence *s2) {
	...
	if (s1->getSize() != params->getFirstSequenceSize()) {
		...
		exc::SequenceMatchingException::genererException(
			"S1 Size error", PARAMETRE_INVALIDE, __LINE__);
	}
	...
	if (s2->getSize() != params->getSecondSequenceSize()) {
		...
		exc::SequenceMatchingException::genererException(
			"S2 Size error", PARAMETRE_INVALIDE, __LINE__);
	}
...
float Correspondence::calcTab(float **tabDifference, int it1, int it2) {
	return params->getMatrixWeight(it1, it2) * params->getValue(tools::Parameters::WEIGHT_DISTANCE) *
...
vector<int> *Correspondence::getIndexOfMinColumn(float **path, int sizeC, int sizeR) {
	...
	int i_min;
	float seuil = params->getValue(tools::Parameters::CDP_THRESHOLD);
	float __min;
	float cost = 0;
	int i = sizeR - 1;
	i_min = i;
...
int Correspondence::getElasticity(int sizeR, int sizeT) {
	int elast = (int) params->getValue(tools::Parameters::FSM_ELASTICITY);
	if (elast < 0) {
...
float Correspondence::getWeight() { 
	return params->getValue(tools::Parameters::FSM_WEIGHT);
}

float Correspondence::getSmallSkipCost(float **tabDiff, int sizeR, int sizeT) {
	float skipValue = params->getValue(tools::Parameters::FSM_SMALL_SKIP_COST);
	if (skipValue < 0) {
		vector<float> tabMin;
		vector<int> tabMinJ;
		...
		for (i = 0; i < sizeR; i++) {
			...
			smallAverage = 0;
			tabMinJ.clear();
			for (k = 0; k < params->getValue(tools::Parameters::FSM_NUMBER_ELEMENTS_BY_LINE); k++) {
				__min = FLT_MAX;
				for (j = 0; j < sizeT; j++) {
			...
			if (smallAverage != 0) {
				tabMin.push_back(smallAverage / params->getValue(tools::Parameters::FSM_NUMBER_ELEMENTS_BY_LINE));
				average += smallAverage / params->getValue(tools::Parameters::FSM_NUMBER_ELEMENTS_BY_LINE);
				size++;
			}
		}

		if (size == 0)
			skipValue = params->getValue(tools::Parameters::FSM_DEFAULT_SKIP_COST);
		else
...
float Correspondence::getSkipCost(float **tabDiff, unsigned int sizeR, unsigned int sizeT) {
	float skipValue = params->getValue(tools::Parameters::FSM_SKIP_COST);
	if (skipValue < 0) {
		vector<float> tabMin;
		vector<int> tabMinJ;
		float stdeviation = 0;
		...
		if (size == 0)
			return params->getValue(tools::Parameters::FSM_DEFAULT_SKIP_COST);
		else
		...
		skipValue =
			average + params->getValue(tools::Parameters::FSM_STANDARD_DEVIATION_NUMBER) * stdeviation;
	}
	return skipValue;
}

float Correspondence::getResultat(float minimun, ResultCorrespondence rc, int nbJump) {
	switch ((int) params->getValue(tools::Parameters::FSM_TYPE_RESULT)) {
	case 0:

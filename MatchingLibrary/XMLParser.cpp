...
void *XMLParser::readFile(void *document) {
	...
	if (type_node == 0) {
		delete sequenceList;
		exc::SequenceMatchingException::genererException(
			"XML Reading error : Type node was not found", ERREUR_FILE, __LINE__);
	}
	...
	do {
		...
		if (sequenceType == CHARACTER) {
			...
			do {
				...
				do {
					// Iterate vector
					rapidxml::xml_attribute<> *vecval_attr = vec_node->first_attribute("value");
					if (vecval_attr == 0) {
						delete sequenceList;
						delete sequence;
						delete vec;
						exc::SequenceMatchingException::genererException(
							"XML Reading error : Invalid value attribute in a VectorElement tag", ERREUR_FILE, __LINE__);
					}

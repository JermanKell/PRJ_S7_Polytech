...
vector<ResultCorrespondence> *LCSCorrespondence::match(model::Sequence *s1,
	model::Sequence *s2) {
	// Initialisation
	...
	while (it < cs1->getSize() + 1) {
		while (it2 < cs2->getSize() + 1) {
			if (params->getValue(tools::Parameters::WEIGHT_DISTANCE) *
...
std::string support::LCSCorrespondence::format(model::Sequence * sTarget, model::Sequence * sRef, ResultCorrespondence * result)
{
	stringstream ret;

...
vector<ResultCorrespondence> *LevenstheinCorrespondence::match(model::Sequence *s1, model::Sequence *s2) {
	...
	while (it < cs1->getSize() + 1) {
		vecValue[it][0] = it * params->getValue(tools::Parameters::LEVENSHTEIN_ADD_COST);
		vecCorrespondance[it][0] = HAUT;
		it = it + 1;
	}
	it = 1;
	while (it < cs2->getSize() + 1) {
		vecValue[0][it] = it * params->getValue(tools::Parameters::LEVENSHTEIN_DEL_COST);
	...
	while (it < cs1->getSize() + 1) {
		while (it2 < cs2->getSize() + 1) {
			...
			float vh =
				params->getMatrixWeight(it - 1, it2 - 1) * vecValue[it][it2 - 1] + params->getValue(tools::Parameters::LEVENSHTEIN_DEL_COST);
			float vg =
				params->getMatrixWeight(it - 1, it2 - 1) * vecValue[it - 1][it2] +
				params->getValue(tools::Parameters::LEVENSHTEIN_ADD_COST);
			float dist = params->getValue(tools::Parameters::WEIGHT_DISTANCE) *

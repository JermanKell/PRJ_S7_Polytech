...
CharacteristicVector::~CharacteristicVector() { delete vCVlist; }
...
void CharacteristicVector::addValue(float elt) { vCVlist->push_back(elt); }
...
int CharacteristicVector::getSize() { return vCVlist->size(); }
...
float CharacteristicVector::distance(Element *eOD1, Element *eOD2) {
	...
	if (c1->vCVlist->size() != 0) {
		...
		while (it < size) {
			ret = ret +
				powf((float)c1->vCVlist->at(it) - (float)c2->vCVlist->at(it), 2);
			it = it + 1;
		}
	}
	return ret;

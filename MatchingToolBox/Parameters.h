...
namespace tools {
	...
	private :
		...
		/*!< Weight applied to an element of the first sequence when we calculate the
		* distance, 1 by default*/
		vector<float> *vPweightS1;
		/*!< Weight applied to an element of the second sequence when we calculate the
		* distance, 1 by default*/
		vector<float> *vPweightS2;
		/*!< Weight applied to a value of a CaracteristicVector when we calculate the
		* distance, 1 by default*/
	...
	public :
		...
		const std::string SIZE_FIRST_SEQUENCE = "iPsizeS1";
		/*!< key of the size of the sequence 2 */
		const std::string SIZE_SECOND_SEQUENCE = "iPsizeS2";
		/*!< The size of the CaracteristicVector, -1 if not a vector*/
		const std::string SIZE_CARACTERISTIC_VECTOR = "iPsizeVecCarac";
		/*!< Weight applied to the distance, 1 par défaut*/
		const std::string WEIGHT_DISTANCE = "vPweigtDistance";
		/*!<The threshold used for the CDP __min value, -1 by default*/
		const std::string CDP_THRESHOLD = "fPThreshold";
		/*!<THe cost for a Levenshtein mouvement, 1 by default*/
		const std::string LEVENSHTEIN_ADD_COST = "fPLevenshteinAddCost";
		const std::string LEVENSHTEIN_DEL_COST = "fPLevenshteinDelCost";
		const std::string LEVENSHTEIN_TRANS_COST = "fPLevenshteinTransCost";
		/*!<The MVM elasticity value, 10 by default*/
		const std::string MVM_ELASTICITY = "iPMvmElastic";
		/*!<The FSM elasticity value, -1 by default*/
		const std::string FSM_ELASTICITY = "iPFsmElesticity";
		/*!<The value of the weight of a jump in FSM, 1 by default*/
		const std::string FSM_WEIGHT = "fPFsmWeight";
		/*!<The value of the small skip cost of a jump in FSM, -1 by default*/
		const std::string FSM_SMALL_SKIP_COST = "fPFsmSmallSkipCost";
		/*!<The value of the skip cost of a jump in FSM, -1 by default*/
		const std::string FSM_SKIP_COST = "fPFsmSkipCost";
		/*!<The value of result wanted for the FSM, 0 by default*/
		const std::string FSM_TYPE_RESULT = "iPFsmTypeOfResult";
		/*!<The number of elements taken for each line in the calcul of the skipcost
		* in the FSM, 2 by default*/
		const std::string FSM_NUMBER_ELEMENTS_BY_LINE = "iPFsmNbOfMinByLine";
		...
		*/
		const std::string FSM_FIRST_COL_FOR_RESULT = "bPFsmFirstColForResult";
		/*!<The value of default skip cost for the FSM, 0.1 by default*/
		const std::string FSM_DEFAULT_SKIP_COST = "fPFsmDefaultSkipCost";
		/*!<The number of standart deviation taken in the calcul of the skip cost for
		* the FSM, 2 by default*/
		const std::string FSM_STANDARD_DEVIATION_NUMBER = "iPFsmStandartDeviationNumber";

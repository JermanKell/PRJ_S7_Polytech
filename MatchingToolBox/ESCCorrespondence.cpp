...
vector<ResultatCorrespondence> *ESCCorrespondence::match(model::Sequence *s1, model::Sequence *s2)
{
	//s1 : target
	//s2 : ref
	// Initialisation
	...
	int tmp = 0, nbJump = 0;
	bool first = true;
	j_min = getIndexOfMinRow(pathCost, cs2->getSize(), cs1->getSize(), params->getValue(params->FSM_FIRST_COL_FOR_RESULT) > 0);
	int i_first;
	int j_first;
...
{
	if (it1 == 0 || it2 == 0)
		return params->getValue(params->WEIGHT_DISTANCE) *(sqrt(tabDifference[it1][it2]));

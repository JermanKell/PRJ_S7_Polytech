...
namespace tools {
	...
	*/
	...
	protected:
		...
		*/
		float getResultat(float minimun, ResultatCorrespondence rc, int nbJump);
	...
	public:
		...
		*/
		virtual vector<ResultatCorrespondence> *match(model::Sequence *sTarget, model::Sequence *sRef) = 0;

		/*!
		* \brief format
		* Abstract function for formatting the result
		* \param result of matching sequences
		* \return format the format of the result based on what algorithm.
		*/
		virtual std::string format(model::Sequence *sTarget, model::Sequence *sRef, ResultatCorrespondence * result);


		std::string format(model::Sequence *sTarget, model::Sequence *sRef, vector<ResultatCorrespondence> * result);
